...
function generateMorrisDonutGraph(dataFromFeed,colourData) {
    Morris.Donut({
...
function setMorrisDonutData(quote){
    ...
    function setData(temp) {
        this.label = temp.label;
        this.value = Math.abs(temp.value);
    ...
    for(i = 0 ;i<investedStocksElements.length;i++){
        ...
        tempData['value'] = (parseFloat(quote[investedStocksElements[i]].change)*(investedStocks[investedStocksElements[i].toLowerCase()])).toFixed(2);
        var temp = new setData(tempData);
        dataArray.push(temp);
        var colorInstance = new setColor(tempData);
        colorArray.push(colorInstance.color);
    }
    console.log("Plotting Donut Graph");
    generateMorrisDonutGraph(dataArray,colorArray);
}
...
//            console.log(data);
            if (textStatus == 'success') {
                ...
                feedsObject.showTopCompanies(companyQuotes);
